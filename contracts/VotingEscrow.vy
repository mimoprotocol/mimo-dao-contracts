...
def _deposit_for(_from: address, _addr: address, _value: uint256, unlock_time: uint256, locked_balance: LockedBalance, type: int128):
    ...
    @param _from Depositer's wallet address
...
@external
@nonreentrant('lock')
def deposit_for(_addr: address, _value: uint256, _wallet: address = ZERO_ADDRESS):
    """
    ...
    """
    assert self.migrator == ZERO_ADDRESS, "Migrate started"
...
@internal
def _create_lock(_from: address, _addr: address, _value: uint256, _unlock_time: uint256):
    assert self.migrator == ZERO_ADDRESS, "Migrate started"
...
def create_lock(_value: uint256, _unlock_time: uint256):
    ...
    """
    self._create_lock(msg.sender, msg.sender, _value, _unlock_time)
...
def create_lock_for(_addr: address, _value: uint256, _unlock_time: uint256):
    ...
    @param _addr Account to deposit
    @param _value Amount to deposit
    @param _unlock_time Epoch time when tokens unlock, rounded down to whole weeks
    """
    self._create_lock(msg.sender, _addr, _value, _unlock_time)
...
def withdraw():
    ...
    assert _locked.amount > 0, "The lock amount is zero"
...
@external
@nonreentrant('lock')
def migrate(_addr: address) -> uint256:
    """
    @notice migrate all tokens by `migrator`
    """
    assert msg.sender == self.migrator, "Only migrator can migrate"
    _locked: LockedBalance = self.locked[_addr]
    assert _locked.amount > 0, "The lock amount is zero"
    value: uint256 = convert(_locked.amount, uint256)
    ...
    assert ERC20(self.token).transfer(self.migrator, value)
