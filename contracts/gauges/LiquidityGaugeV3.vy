...
interface Minter:
    ...
    def future_change_time_write() -> uint256: nonpayable
...
def __init__(_lp_token: address, _minter: address, _admin: address):
    ...
    self.name = concat("MIMO_ ", symbol, " Gauge Deposit")
    self.symbol = concat(symbol, "-gauge")

    mimo_token: address = Minter(_minter).token()
    controller: address = Minter(_minter).controller()

    self.lp_token = _lp_token
    self.minter = _minter
    self.admin = _admin
    self.mimo_token = mimo_token
    self.controller = controller
    self.voting_escrow = Controller(controller).voting_escrow()

    self.period_timestamp[0] = block.timestamp
    self.inflation_rate = Minter(_minter).rate()
    self.future_epoch_time = Minter(_minter).future_change_time_write()
...
def _update_liquidity_limit(addr: address, l: uint256, L: uint256):
    """
    @notice Calculate limits which depend on the amount of MIMO token per-user.
            Effectively it calculates working balances to apply amplification
            of MIMO production by MIMO
...
def _checkpoint(addr: address):
    ...
    if prev_future_epoch >= _period_time:
        _token: address = self.mimo_token
        self.future_epoch_time = Minter(self.minter).future_change_time_write()
        new_rate = Minter(self.minter).rate()
...
def reward_contract() -> address:
    """
    @notice Address of the reward contract providing non-MIMO incentives for this gauge
...
def set_killed(_is_killed: bool):
    ...
    @dev When killed, the gauge always yields a rate of 0 and so cannot mint MIMO
